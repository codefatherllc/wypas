...
# Usage: stage-bootstrap.sh <wypas_exe> <pack_dir> <out_dir>
#
# Windows counterpart of the DMG/AppImage thin bundle (packaging/linux/
# package.sh, packaging/macos/package-dmg.sh): stage ONLY the bootstrap the
# updater window needs — data/ + corelib + updater + its UI deps + a rendered
# updater-ON init.lua — then ENC3-encrypt the staged tree in place with the
# release binary itself. ISCC bundles the tree into setup.exe, so the
# installer performs NO network I/O at install time: the in-game updater
# (guaranteed present here) syncs the rest of the pack checksum-verified on
# first boot.
#
# Runs on the windows GH runner under git-bash. Pass relative paths only:
# they reach the native exe unconverted, and Windows accepts forward slashes.
...
# plaintext init.lua for the seed-gate check — kept OUTSIDE the staging dir so
# it can never be swept into the installer.
PLAIN_INIT="$(mktemp)"
trap 'rm -f "$PLAIN_INIT"' EXIT

rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"

echo "==> Staging THIN bootstrap from $PACK_DIR"
...
# render as the DMG/AppImage and the wypas-proxy served pack.
...
sed -e "s|__BASE_URL__|${WYPAS_BASE_URL}|g" \
    ...
    "$TMPL" > "$OUT_DIR/init.lua"
if ! grep -q "${WYPAS_BASE_URL}/api/updater" "$OUT_DIR/init.lua"; then
...
cp "$OUT_DIR/init.lua" "$PLAIN_INIT"
...
# under the target dir. Bounded: a binary lacking WITH_ENCRYPTION falls
# through --encrypt into normal startup and would sit open in the runner's
# desktop session forever.
echo "==> Encrypting pack in place: $WYPAS_EXE --encrypt $OUT_DIR"
"$WYPAS_EXE" --encrypt "$OUT_DIR" &
ENC_PID=$!
...
if [ "$(head -c 4 "$OUT_DIR/init.lua" 2>/dev/null)" != "ENC3" ]; then
  echo "ERROR: init.lua is not ENC3-encrypted after --encrypt." >&2
  echo "       wypas.exe must be built WITH_ENCRYPTION with ASSET_ENCRYPTION_SEED set." >&2
  exit 1
fi
enc_count=$(find "$OUT_DIR/data" "$OUT_DIR/modules" \
  -type f 2>/dev/null -exec sh -c '[ "$(head -c 4 "$1" 2>/dev/null)" = ENC3 ]' _ {} \; -print | wc -l | tr -d ' ')
echo "==> Pack encrypted (${enc_count} ENC3 files staged)"
rm -f "$OUT_DIR/encryption.log"

# Seed verification (same check as the DMG/AppImage): recover the seed from
# the ENC3 adler word of init.lua (known plaintext); reject seed-0 (lenient
# build, universally decryptable) and wrong-seed (won't match the prod pack).
if [ -n "${ASSET_ENCRYPTION_SEED:-}" ] && command -v python3 >/dev/null 2>&1; then
  if ! ASSET_ENCRYPTION_SEED="$ASSET_ENCRYPTION_SEED" python3 - \
      "$OUT_DIR/init.lua" "$PLAIN_INIT" <<'PY'
...
if recovered == 0:
    sys.stderr.write("ERROR: pack encrypted with seed 0 (lenient) — universally decryptable, "
                     "no protection. Build wypas.exe with -DASSET_ENCRYPTION_SEED.\n"); sys.exit(1)
